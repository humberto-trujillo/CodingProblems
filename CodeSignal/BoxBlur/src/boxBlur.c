...
{
    arr_integer a = {len, malloc(sizeof(int) * len)};
    return a;
}

arr_arr_integer alloc_arr_arr_integer(int len)
{
    arr_arr_integer a = {len, malloc(sizeof(arr_integer) * len)};
    return a;
}
...
arr_arr_integer boxBlur(arr_arr_integer image)
{
    ...
    for(int i=1; i<image.size-1; i++)
    {
        ...
        for (int j=1; j<image.arr[i].size-1; j++)
        {
            blur.arr[i-1].arr[j-1] = mean(image, i, j);
        }
    }
    return blur;
}
